...
export const DatasetForm = function(id = `datasetForm`, events = {}) {
	let self = this;
	this.id = id;
	...
	$(`#${this.id} div[key="dataset\\.id"], #${this.id} div[key="dataset\\.label"]`)
		.parent()
		.click(function() {
			if (typeof self.events.onDatasetIdClick === `function`)
				return self.events.onDatasetIdClick(self.getDataset());
		});
	// onDatasetDoneClick
	$(`#${this.id} button[name="datasetForm\\.done"]`).click(function() {
		if (typeof self.events.onDatasetDoneClick === `function`)
			return self.events.onDatasetDoneClick(self.getDataset());
	});
	...
	this.properties = {
		...
		reuse: function(value, inputs = false) {
			...
			if (inputs)
				self.container
					.find(`input[type="checkbox"][name="datasetForm\\.reuse"]`)
					.prop(`checked`, reuse);
		...
		dataType: function(value, inputs = false) {
			...
			let option = self.container.find(
				`select[name="datasetForm\\.dataType"] option[value="${value}"]`
			);
			if (inputs) {
			...
			if (option.length === 0) {
				...
				self.container
					.find(`select[name="datasetForm\\.dataType"] option:first-child`)
					.prop(`selected`, true);
		...
		subType: function(value, inputs = false) {
			...
			if (inputs)
				self.container
					.find(`select[name="datasetForm\\.subType"] option[value="${value}"]`)
					.prop(`selected`, true);
		...
		customDataType: function(value, inputs = false) {
			...
			if (inputs)
				self.container
					.find(`input[type="text"][name="datasetForm\\.customDataType"]`)
					.val(value);
		...
		description: function(value) {
			...
			self.container
				.find(`div[key="dataset\\.description"]`)
				.attr(`value`, value)
				.html(text);
		...
		bestDataFormatForSharing: function(value) {
			...
			let text = value === `` ? `n/a` : value;
			self.container
				.find(`div[key="dataset\\.bestDataFormatForSharing"]`)
				.attr(`value`, value)
				.html(text);
		...
		mostSuitableRepositories: function(value) {
			...
			let text = value === `` ? `n/a` : value;
			self.container
				.find(`div[key="dataset\\.mostSuitableRepositories"]`)
				.attr(`value`, value)
				.html(text);
		...
		highlight: function(value, inputs = false) {
			...
			if (inputs)
				self.container
					.find(`input[type="checkbox"][name="datasetForm\\.highlight"]`)
		...
		notification: function(value, inputs = false) {
			...
			self.container
				.find(`div[key="dataset\\.notification"]`)
				.attr(`value`, value)
				.text(value);
			if (inputs)
				self.container
					.find(`input[type="text"][name="datasetForm\\.notification"]`)
					.val(value);
			self.dataset.notification = value;
			// input change behaviors
			// Hide notification if empty, else show
			if (!self.dataset.notification)
				self.container.find(`div[key="dataset\\.notification"]`).hide();
		...
		name: function(value, inputs = false) {
			...
			if (inputs)
				self.container.find(`input[type="text"][name="datasetForm\\.name"]`).val(value);
		...
		DOI: function(value, inputs = false) {
			...
			if (inputs)
				self.container.find(`input[type="text"][name="datasetForm\\.DOI"]`).val(value);
...
DatasetForm.prototype.setRepos = function(cb) {
	...
	let subType = this.dataset.subType
		? this.getDatatypeInfos(this.dataset.dataType, this.dataset.subType)
		: undefined;
...
DatasetForm.prototype.updateLabel = function(value = ``) {
	...
	// Update label
	this.container
		.find(`div[key="dataset\\.label"]`)
		.attr(`value`, data)
		.text(data);
	this.container
		.find(`div[key="dataset\\.tab"][dataset-id="${this.dataset.id}"]`)
		.attr(`value`, data)
		.text(data);
};
...
DatasetForm.prototype.getDataset = function() {
	return {
		...
		bestPracticeForIndicatingReUseOfExistingData: this.dataset
	...
	keys.map(function(key) {
		...
		properties.map(function(property) {
			if (typeof property === `string`)
				result[key][property] = self.getResourceOf(self.dataset[key], property);
...
DatasetForm.prototype.getDatatypeInfos = function() {
	let result = {},
		properties = [
			`label`,
			`description`,
...
DatasetForm.prototype.refreshDatatypeView = function() {
	if (!this.dataset.dataType || (!this.dataset.dataType && !this.dataset.subType))
		this.container
			.find(`div[key="dataset\\.customDataType"]`)
			.parent()
			.show();
	else
		this.container
...
DatasetForm.prototype.setSubtypes = function() {
	let data = this.dataset.dataType
...
DatasetForm.prototype.setView = function(opts = {}) {
	if (opts.isCurator) {
		this.container
			...
			.show();
		this.container
			.find(`button[name="datasetForm\\.refreshDatatypes"]`)
			.parent()
			.show();
		this.container
			.find(`input[type="checkbox"][name="datasetForm\\.highlight"]`)
...
DatasetForm.prototype.getIconOfStatus = function(status) {
	let i = $(`<i>`);
	if (status === `valid`)
		i.addClass(`fas fa-check success-color-dark`).attr(`title`, `This dataset is valid`);
	else if (status === `saved`)
		i.addClass(`far fa-save success-color-dark`).attr(`title`, `This dataset is saved`);
	else if (status === `modified`)
		i.addClass(`fas fa-pen warning-color-dark`).attr(`title`, `This dataset is modified`);
...
// update dataset
DatasetForm.prototype.updateSentence = function() {
	// Set properties
...
DatasetForm.prototype.addTab = function(dataset, opts) {
	...
	newTab
		.find(`div[key="dataset\\.tab"]`)
		.attr(`value`, dataset.id)
		.attr(`dataset-id`, dataset.id)
		.text(dataset.id);
	newTab
		.find(`div[key="dataset\\.label"]`)
		.attr(`value`, dataset.id)
...
DatasetForm.prototype.refreshDataset = function(data = {}, opts = {}, callback) {
	...
	return typeof callback === `function`
		? callback(null, { shouldSave: shouldSave, dataset: this.dataset })
		: undefined;
};
