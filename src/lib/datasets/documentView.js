...
PdfViewer.prototype.onScroll = function (scrollInfos, direction) {
  ...
  if (direction > 0) {
    ...
    this.renderPreviousPage(function () {
      self.refreshScrollCursor(scrollInfos);
    });
  }
};
...
PdfViewer.prototype.renderUntilPage = function (numPage, cb) {
  ...
  return async.eachSeries(
    ...
    function (numPage, callback) {
      return self.renderPage({ numPage: numPage }, function (err) {
        return callback(err);
      });
    },
    function (err) {
...
PdfViewer.prototype.refresh = function (cb) {
  if (!self.pdfDocument) {
    ...
    return async.eachSeries(
      this.getPages(),
      function (numPage, callback) {
        return self.renderPage({ numPage: numPage, force: true }, function (err) {
          console.log(err);
          return callback(err);
        });
      },
...
XmlViewer.prototype.selectSentence = function (sentence) {
  this.viewer.find(`s[xml\\:id="${sentence.id}"]`).removeClass(`hover`).addClass(`selected`);
};

// Unselect a sentence
XmlViewer.prototype.unselectSentence = function (sentence) {
  this.viewer.find(`s[xml\\:id="${sentence.id}"]`).removeClass(`selected`);
};
