...
export const DatasetsList = function(id = 'datasetsList', events = {}) {
	let self = this;
	this.id = id;
	...
	$(`#${this.id} #datasetsList\\.container\\.bulkActions\\.newDataset`).click(function() {
		if (typeof self.events.onNewDatasetClick === 'function')
			return self.events.onNewDatasetClick();
	});
...
DatasetsList.prototype.load = function(datasets = [], cb) {
	...
	return async.eachSeries(
		...
		function(dataset, callback) {
			self.add(dataset);
			if (typeof self.events.onDatasetLoaded === 'function')
				self.events.onDatasetLoaded(dataset);
			return callback();
		},
		function(err) {
			return cb(err);
...
DatasetsList.prototype.unload = function(datasets, cb) {
	...
	return async.eachSeries(
		...
		function(dataset, callback) {
			self.delete(dataset);
			if (typeof self.events.onDatasetUnloaded === 'function')
				self.events.onDatasetUnloaded(dataset);
			return callback();
		},
		function(err) {
			return cb(err);
...
DatasetsList.prototype.add = function(dataset) {
	if (
		!Array.isArray(dataset.sentences) ||
		!dataset.sentences.length ||
		typeof dataset.sentences[0] === 'undefined'
	)
		return;
	let self = this,
			...
			checked: $('<div>')
				...
				.append(
					$('<i>')
						...
						.attr('title', 'This Dataset is not in the selection')
				),
			link: $('<button>')
	...
	elements.link.click(function(event) {
		...
		if (typeof self.events.onDatasetLink === 'function')
			return self.events.onDatasetLink({
				id: dataset.id,
	...
	elements.delete.click(function(event) {
		...
		if (typeof self.events.onDatasetDelete === 'function')
			return self.events.onDatasetDelete({
				id: dataset.id,
				sentences: dataset.sentences,
				checked: isChecked
			});
	...
	elements.checked.click(function(event) {
		...
		if (typeof self.events.onDatasetCheck === 'function')
			return self.events.onDatasetCheck({
				id: dataset.id,
				sentences: dataset.sentences,
				checked: isChecked
			});
...
DatasetsList.prototype.merge = function() {};
