...
const randomColor = window.randomColor;
...
DocumentHandler.prototype.selectSentence = function (opts, cb) {
  ...
  return this.documentView.scrollToSentence({ sentence: opts.sentence, noAnim: opts.noAnim }, function () {
  ...
  if (datasets.length > 1) {
    ...
    return async.mapSeries(
      ...
      function (item, callback) {
        ...
        return self.deleteDataset(dataset.id, function (err) {
...
DocumentHandler.prototype.synchronize = function () {
  ...
  if (this.documentView) {
    ...
    this.documentView.attach(`onDatasetClick`, function () {});
  ...
  if (this.datasetsList) {
    ...
    this.datasetsList.attach(`onDatasetLoaded`, function () {
      // console.log(dataset);
    });
    ...
    this.datasetsList.attach(`onDatasetCheck`, function () {
      // console.log(dataset);
    });
    ...
    this.datasetsList.attach(`onDatasetLink`, function (dataset) {
      ...
      if (selectedSentences.length === 0)
        ...
        return self.newLinks({ dataset: { id: dataset.id }, sentences: selectedSentences }, function (err) {
